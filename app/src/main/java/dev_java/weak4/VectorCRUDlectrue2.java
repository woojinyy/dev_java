...
public class VectorCRUDlectrue2 {
	...
	public int deptUpdate(int deptno, String user_dname, String user_loc) {
		...
		int result = 0;// 1이면 성공, 0이면 실패
		here://라벨문
		for(int i=0;i<vdept.size();i++){//벡터
			String row[] = vdept.get(i);//i는 로우 인덱스이다
			//if() 여기는 벡터에서 꺼낸 배열을 갖는다.
			for(int j=0;j<row.length;j++){//String[]돌림 1차배열
				...
				if(deptno==deptno2){// 10 부서번호같니
					String[] updRow = {oneRow[0], user_dname, user_loc};
					//수정이라 말하고 입력으로 처리한다 한 셀씩  수정하기 싫다 
					vdept.remove(i);//i번째 자리를 삭제하고
					vdept.add(i, updRow);//삭제된 자리에 추가함 배열은 중간에 끼워넣기가 안된다 사이즈도 못바꾼다.
					//vector를 사용하면 배열의 모든 단점을 보완함-고정,타입단점
					//Vector<Object>
					break here;//break문만 사용시 이중for문에서 내부 for문만을 탈출하게 되는데
					//여기서는 외부 for문을 빠져나가야 하므로 라벨문을 붙여 처리한다.
				}else{
					System.out.println("선택한 부서번호와 벡터에서 가져온 부서번호가 다르다");
				}
			}//end of String[] -inner for문
		}//end of Vector-바깥쪽  for문에 라벨문을 사용할것
		getDeptList();//수정이 된 값을 보자
		return result;
	}
	...
	public static void main(String[] args) {
		VectorCRUDlectrue2 vCrud = new VectorCRUDlectrue2();
		// 입력테스트
		int deptno = 10;
		String dname = "개발부";
		String loc = "인천";
